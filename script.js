/**
* ПРАКТИЧЕСКАЯ РАБОТА №4: ОБЪЕКТЫ И МАССИВЫ. МЕТОДЫ ПЕРЕБОРА МАССИВОВ
* Студент: Александр
* Группа: 2509
*/
...
// 1. СТАРТОВЫЕ ДАННЫЕ (Массив объектов)
...
// 2. ПОИСК ЭЛЕМЕНТОВ DOM (Все обработчики только через addEventListener)
...
btnSearch.addEventListener('click', () => {
    // ВАЛИДАЦИЯ: убираем лишние пробелы (trim) и приводим к нижнему регистру для нечувствительного поиска
    const searchText = searchInput.value.toLowerCase().trim();
    const selectedCategory = categorySelect.value; // Получаем текущее значение из выпадающего списка

    // Цепочка методов для обработки данных (Подсказка практической работы)
...
// Находим элементы управления из Секции 4 
...
btnSave.addEventListener('click', () => {
    ...
    // ВЫВОД В КОНСОЛЬ (согласно техническому требованию работы):
...
btnLoad.addEventListener('click', () => {
    ...
    // ПРОВЕРКА : Если данных в памяти нет, getItem вернет null
    if (savedData) {
...
btnStats.addEventListener('click', () => {
    ...
    // Рассчитываем среднее и округляем до 2 знаков после запятой (метод toFixed)
    ...
    // а специальная структура Set автоматически удалит все дубликаты.
    ...
    // Вывод аналитики в консоль для сверки с условиями работы
    console.log("--- СТАТИСТИКА ---");
    console.log(`Средняя цена: ${avgPrice} BYN (Ожидалось: 541.25)`);
    console.log(`Категории:`, uniqueCategories);
});

// ПРОВЕРКА: В коде использованы только строгие сравнения ===, нет старого атрибута inline-onclick, 
